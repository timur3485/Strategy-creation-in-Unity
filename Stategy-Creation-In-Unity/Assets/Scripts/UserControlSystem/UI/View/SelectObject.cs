...
public class SelectObject : MonoBehaviour
{
    ...
    [SerializeField]
    private SelMode SelectionMode = SelMode.AndChildren;
    [Tooltip("The last two type will require rendering an extra Camera Depth Texture.")]
    [SerializeField]
    private OutlineMode OutlineType = OutlineMode.ColorizeOccluded;
    [Tooltip("Decide whether the alpha data of the main texture affect the outline.")]
    [SerializeField]
    private AlphaType AlphaMode = AlphaType.KeepHoles;
    private Renderer TargetRenderer, lastTarget;
    private Renderer[] ChildrenRenderers;
    [ColorUsageAttribute(true, true)]
    private Color OutlineColor = new Color(1f, 0.55f, 0f), OccludedColor = new Color(0.5f, 0.9f, 0.3f);
    [Range(0, 1)]
    private float OutlineWidth = 0.2f;
    [Range(0, 1)]
    private float OutlineHardness = 0.85f;
    ...
    public void Action(RaycastHit[] raycastHits)
    {
        ClearTarget();
        if (raycastHits == null)
            return;
